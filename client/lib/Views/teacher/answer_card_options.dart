...
class AnswerCardOptionsState extends State<AnswerCardOptions> {
  ...
  void fetchData() {
    teacherNotifier
      ...
      position: position,
        ...
        .then((value) {
      setState(() {
        teacherNotifier.scantronSolutionListModel = ScantronSolutionModel().fromJsonList(jsonEncode(value.data));
        selected = List<bool>.generate(teacherNotifier.scantronSolutionListModel.length, (int index) => false);
  ...
  List<DataRow> generateList() {
    return List<DataRow>.generate(
      teacherNotifier.scantronSolutionListModel.length,
      (int index) => DataRow(
        ...
        cells: <DataCell>[
          DataCell(Text(overflow: TextOverflow.ellipsis, maxLines: 1, teacherNotifier.scantronSolutionListModel[index].id.toString())),
          DataCell(
            Text(
              overflow: TextOverflow.ellipsis,
              maxLines: 1,
              teacherNotifier.scantronSolutionListModel[index].option == 'none' ? '' : teacherNotifier.scantronSolutionListModel[index].option,
            ),
          ),
          DataCell(
            Text(
              overflow: TextOverflow.ellipsis,
              maxLines: 1,
              widget.questionType < 4 ? checkCorrectAnswer(teacherNotifier.scantronSolutionListModel[index].correctAnswer) : '',
            ),
          ),
          DataCell(Text(overflow: TextOverflow.ellipsis, maxLines: 1, teacherNotifier.scantronSolutionListModel[index].correctItem)),
          DataCell(
            Text(
              overflow: TextOverflow.ellipsis,
              maxLines: 1,
              checkPosition(teacherNotifier.scantronSolutionListModel[index].position),
            ),
          ),
          DataCell(Text(overflow: TextOverflow.ellipsis, maxLines: 1, teacherNotifier.scantronSolutionListModel[index].candidateAnswer)),
          DataCell(Text(overflow: TextOverflow.ellipsis, maxLines: 1, teacherNotifier.scantronSolutionListModel[index].scoreRatio.toString())),
          DataCell(
            Row(
              children: [
          ...
          DataCell(Text(overflow: TextOverflow.ellipsis, maxLines: 1, Tools().timestampToStr(teacherNotifier.scantronSolutionListModel[index].createTime))),
          DataCell(Text(overflow: TextOverflow.ellipsis, maxLines: 1, Tools().timestampToStr(teacherNotifier.scantronSolutionListModel[index].updateTime))),
        ],
  ...
  void viewImage({required String attachment, bool big = false}) {
    ...
    setState(() {
      teacherNotifier.teacherScantronSolutionViewAttachments(optionAttachment: attachment).then((value) {
        if (value.data != null) {
          showDialog(
            context: context,
  ...
  onSortColum(int columnIndex, bool ascending) {
    if (columnIndex == 0) {
      if (ascending) {
        teacherNotifier.scantronSolutionListModel.sort((a, b) => a.id.compareTo(b.id));
      } else {
        teacherNotifier.scantronSolutionListModel.sort((a, b) => b.id.compareTo(a.id));
      }
    }
    // 重置全选
    selected = List<bool>.generate(
      teacherNotifier.scantronSolutionListModel.length,
      (int index) {
        teacherNotifier.scantronSolutionListModel[index].selected = false;
        showSelected = 0;
        return false;
      },
    );
